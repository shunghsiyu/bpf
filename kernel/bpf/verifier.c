...
static void __reg_deduce_mixed_bounds(struct bpf_reg_state *reg)
{
	 ...
	 */
	if (reg->val.s32_min >= 0 && reg->val.smin >= S32_MIN && reg->val.smax <= S32_MAX) {
		reg->val.smin = reg->val.s32_min;
		reg->val.smax = reg->val.s32_max;
		reg->val.umin = reg->val.s32_min;
		reg->val.umax = reg->val.s32_max;
		reg->val.var_off = tnum_intersect(reg->val.var_off,
					      tnum_range(reg->val.smin, reg->val.smax));
	}
}
				...
				s32 s32_max)
{
	...
	reg->val.s32_min = max_t(s32, reg->val.s32_min, s32_min);
	reg->val.s32_max = min_t(s32, reg->val.s32_max, s32_max);

	reg->val.smin = max_t(s64, reg->val.smin, s32_min);
	reg->val.smax = min_t(s64, reg->val.smax, s32_max);
 ...
 *    R5_w=inv(id=0,val.umax=1951,var_off=(0x0; 0x7ff))
 * r5 += 1
 * ...
				   ...
				   bool zero_size_allowed)
{
	 ...
	 * need to try adding each of min and max to off
...
static void coerce_reg_to_size_sx(struct bpf_reg_state *reg, int size)
{
	...
	if ((s64_max >= 0) == (s64_min >= 0)) {
		reg->val.s32_min = reg->val.smin = s64_min;
		reg->val.s32_max = reg->val.smax = s64_max;
		reg->val.u32_min = reg->val.umin = s64_min;
		reg->val.u32_max = reg->val.umax = s64_max;
...
static void set_sext32_default_val(struct bpf_reg_state *reg, int size)
{
	...
	reg->val.u32_min = 0;
	reg->val.u32_max = U32_MAX;
	reg->val.var_off = tnum_subreg(tnum_unknown);
}
...
static void coerce_subreg_to_size_sx(struct bpf_reg_state *reg, int size)
{
	...
	if ((s32_min >= 0) == (s32_max >= 0)) {
		...
		reg->val.var_off = tnum_subreg(tnum_range(s32_min, s32_max));
		return;
	}
	...
	if (reg->type == PTR_TO_MAP_KEY) {
		...
		if (tnum_is_const(reg->val.var_off))
			kptr_field = btf_record_find(reg->map_ptr->record,
						     off + reg->val.var_off.value, BPF_KPTR | BPF_UPTR);
		if (kptr_field) {
			      ...
			      struct bpf_call_arg_meta *meta)
{
	...
	err = check_helper_mem_access(env, regno - 1, reg->val.umax,
...
{
	if (return_32bit)
		return range.minval <= reg->val.s32_min && reg->val.s32_max <= range.maxval;
	else
					...
					int argno)
{
	...
	WARN_ON_ONCE(is_kfunc_release(meta) &&
		     (reg->off || !tnum_is_const(reg->val.var_off) ||
		      reg->val.var_off.value));
				   ...
				   const struct bpf_reg_state *off_reg)
{
	...
	case BPF_SUB:
		 ...
		 */
		if (check_sub_overflow(smin_ptr, smax_val, &dst_reg->val.smin) ||
		    check_sub_overflow(smax_ptr, smin_val, &dst_reg->val.smax)) {
				  ...
				  struct bpf_reg_state *src_reg)
{
	 ...
	 * dst_reg val.var_off to refine the result.
	 */
	dst_reg->val.u32_min = 0;
	dst_reg->val.u32_max = U32_MAX;
